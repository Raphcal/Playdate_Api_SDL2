...
static void save(MELSprite * _Nonnull sprite, MELOutputStream * _Nonnull outputStream) {
    ...
    if (self.instance && currentScene->type == SceneTypeGame) {
        GameScene *gameScene = (GameScene *)currentScene;
        const int waveIndex = self.instance->mapName - MapNameWave0Tutorial;
        if (waveIndex >= 0 && waveIndex < gameScene->waves.count) {
            Wave *wave = gameScene->waves.memory + waveIndex;
            MELSpriteInstanceList instances = wave->instances;
            const int instanceIndex = (int) (self.instance - instances.memory);
            if (instanceIndex >= 0 && instanceIndex < instances.count) {
                ...
                MELOutputStreamWriteInt(outputStream, instanceIndex);
            } else {
                playdate->system->logToConsole("Invalid instance index, expected [0 ..< %d] but was: %d", instances.count,  instanceIndex);
                MELOutputStreamWriteBoolean(outputStream, false);
            }
        } else {
            playdate->system->logToConsole("Invalid wave index, expected [0 ..< %d] but was: %d", gameScene->waves.count,  waveIndex);
            MELOutputStreamWriteBoolean(outputStream, false);
        }
    } else {
        MELOutputStreamWriteBoolean(outputStream, false);
...
static MELSprite * _Nullable load(MELInputStream * _Nonnull inputStream, LCDSprite * _Nonnull sprite) {
    ...
    MELBoolean hasInstance = MELInputStreamReadBoolean(inputStream);
    if (hasInstance && (!currentScene || currentScene->type != SceneTypeGame)) {
        playdate->system->error("Unable to load instance, currentScene is NULL or not a GameScene type");
    } else if (hasInstance) {
        GameScene *gameScene = (GameScene *)currentScene;
        const int waveIndex = MELInputStreamReadInt(inputStream);
        Wave *wave = gameScene->waves.memory + waveIndex;

        const int instanceIndex = MELInputStreamReadInt(inputStream);
        self->instance = wave->instances.memory + instanceIndex;
    }
