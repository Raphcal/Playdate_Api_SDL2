...
LCDSprite * _Nullable TextConstructorArgInt(MELPoint origin, LCDFont * _Nonnull font, LCDBitmapDrawMode drawMode, MELHorizontalAlignment alignment, const char * _Nonnull format, int arg) {
    LCDSprite *sprite = NULL;
    char *text = NULL;
    playdate->system->formatString(&text, format, arg);
    if (text) {
        const unsigned int length = (unsigned int) strlen(text);
        sprite = ImageConstructor(origin, drawText(font, drawMode, 0, text, length));
        free(text);
...
LCDSprite * _Nullable TextConstructorArgFloat(MELPoint origin, LCDFont * _Nonnull font, LCDBitmapDrawMode drawMode, MELHorizontalAlignment alignment, const char * _Nonnull format, float arg) {
    LCDSprite *sprite = NULL;
    char *text = NULL;
